        ...
        task.spawn(function()
            ...
            for aA, B in pairs(_) do
                if game.PlaceId ~= B and al.HPHalf then
                    H.Character.HealthProperties.Health:GetPropertyChangedSignal('Value'):Connect(function()
                        pcall(function()
                            if IsAlive() and H.Character.HealthProperties.Health.Value /
                                H.Character.HealthProperties.MaxHealth.Value * 100 < 50 then
                                a6, a7, a4, a9 = 50, 40, 1, 3;
                                repeat
                                    wait(1)
                                until H.Character.HealthProperties.Health.Value /
                                    H.Character.HealthProperties.MaxHealth.Value * 100 > 90;
                                if aC() then
                                    a6, a7, a9 = 36, 30, 6
                                elseif ay() then
                                    a6, a7, a9 = .1, 14, 5
...
local c2 = bF:Toggle({
    ...
    Callback = function(aJ)
        ...
        task.spawn(function()
            ...
            j.InputBegan:connect(function(c4)
                if al.InfJump and c4.UserInputType == Enum.UserInputType.Keyboard and c4.KeyCode == Enum.KeyCode.Space then
                    InfJump(K, function(H)
                        if H:GetState() == Enum.HumanoidStateType.Jumping or H:GetState() ==
                            Enum.HumanoidStateType.Freefall then
                            InfJump(J, function(bZ)
...
local ReduceLag = bG:Toggle({
    ...
    Callback = function(aJ)
        ...
        task.spawn(function()
            ...
            e.DescendantAdded:Connect(function(c7)
            ...
            e.ChildAdded:Connect(function(c8)
...
local cb = bG:Toggle({
    ...
    Callback = function(aJ)
        ...
        task.spawn(function()
            ...
            cc:GetPropertyChangedSignal('Enabled'):Connect(function()
                if cc.Enabled then
                    P:SkipCutscene()
                end
            end)
...
local cd = bG:Toggle({
    ...
    Callback = function(aJ)
        ...
        task.spawn(function()
            ak('WZ_Toggles', al)
            local bD;
            bD = e.Mobs.ChildAdded:connect(function(bo)
            ...
            for aA, B in pairs(_) do
                if game.PlaceId ~= B and al.RepeatRaid then
                    ...
                    H.PlayerGui.TowerFinish.TowerFinish.Description.Overlay:GetPropertyChangedSignal('Text'):Connect(
                        function()
                            if H.PlayerGui.TowerFinish.TowerFinish.Description.Overlay.Text ==
                                'Collect your rewards! (10)' then
                                S:SetLeaveChoice(H, true)
                                S:NotifyReadyToLeave(H)
                            end
                        end)
...
local ck = bI:Toggle({
    ...
    Callback = function(aJ)
        ...
        task.spawn(function()
            ak('WZ_Toggles', al)
            d.PlayerRemoving:connect(function(cl)
...
bK:Button({
    ...
    Callback = function(cn)
        if cn then
            e.Mobs.ChildAdded:connect(function(co)
...
local dg;
dg = k.Profiles[H.Name].Inventory.Items.DescendantAdded:Connect(function()
    for A, B in pairs(k.Profiles[H.Name].Inventory.Items:GetChildren()) do
        if B:FindFirstChild('UpgradeLimit') and B:FindFirstChild('Level') and
            not B:FindFirstChild('Item Already Exists') then
            wait(.25)
            local dh = false;
...
local dB = {{
    ...
    ['fields'] = {{
        ...
        ['name'] = 'Level: ' .. tostring(string.match(H.PlayerGui.Hotbar.Hotbar.Vitals.Level.TextLabel.Text, '%d+')),
...
getgenv().PlayerData = h:JSONEncode({
    ['content'] = '',
    ['embeds'] = {unpack(dB)}
})
